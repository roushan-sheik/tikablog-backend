...
class AuthHelper {
  ...
  static isExpired(timestamp: Date | number | null | undefined): boolean {
    return (
      !timestamp ||
      (typeof timestamp === "object" ? timestamp.getTime() : timestamp) <
        Date.now()
    );
  }
  ...
  static minutesUntil(futureTime: Date | number): number {
    const futureTimeMs =
      typeof futureTime === "object" ? futureTime.getTime() : futureTime;
   ...
   */
  static validateEmailAndOTP(
    { email, otp }: EmailOTPData,
    next: NextFunction
  ): void {
    if (!email || !otp)
  ...
  static validateUserActive(user: User | null, next: NextFunction): void {
    ...
    if (user.isDeleted)
      return next(new ApiError("Account has been deleted", 400));
    if (user.verify_otp)
      return next(new ApiError("Account is already verified", 409));
    if (user.isActive)
      return next(new ApiError("User account is already active", 400));
    return next(
      new ApiError("Please verify your account before logging in.", 400)
    );
  }
  ...
  static checkOtpResendAllowed(user: User, next: NextFunction): void {
    if (user.otpExpires && this.isExpired(user.otpExpires) === false) {
      ...
      return next(
        new ApiError(
          `Please wait ${minutesLeft} minute(s) before requesting a new OTP`,
          429
        )
      );
    }
  ...
  static checkPasswordResetTokenExpiry(user: User, next: NextFunction): void {
    if (
      ...
      this.isExpired(user.passwordResetExpires) === false
    ) {
      return next(
        new ApiError(
          "You already have a valid reset token. Please use that or wait until it expires.",
          429
        )
      );
    }
